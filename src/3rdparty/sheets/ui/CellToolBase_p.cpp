...
void CellToolBase::Private::updateActions(const Cell& cell)
{
    ...
    //AFA static_cast<KFontAction*>(q->action("font"))->setFont(style.fontFamily());
    //AFA static_cast<KFontSizeAction*>(q->action("fontSize"))->setFontSize(style.fontSize());
...
bool CellToolBase::Private::processHomeKey(QKeyEvent* event)
{
    ...
    if (q->editor()) {
        ...
        QGuiApplication::sendEvent(q->editor()->widget(), event);
        return false;
    } else {
        QPoint destination;
...
bool CellToolBase::Private::formatKeyPress(QKeyEvent * _ev)
{
    ...
    case Qt::Key_Dollar:
        ...
        //AFA command->setPrecision(q->selection()->activeSheet()->map()->calculationSettings()->locale()->monetaryDecimalPlaces());
        break;
    ...
    case Qt::Key_Ampersand:
        ...
        //AFA command->setTopBorderPen(QPen(q->canvas()->resourceManager()->foregroundColor().toQColor(), 1, Qt::SolidLine));
        //AFA command->setBottomBorderPen(QPen(q->canvas()->resourceManager()->foregroundColor().toQColor(), 1, Qt::SolidLine));
        //AFA command->setLeftBorderPen(QPen(q->canvas()->resourceManager()->foregroundColor().toQColor(), 1, Qt::SolidLine));
        //AFA command->setRightBorderPen(QPen(q->canvas()->resourceManager()->foregroundColor().toQColor(), 1, Qt::SolidLine));
        break;
...
void CellToolBase::Private::paintSelection(QPainter &painter, const QRectF &viewRect)
{
    ...
    QLineF line;
    QPen pen(QGuiApplication::palette().text().color(), q->canvas()->viewConverter()->viewToDocumentX(2.0));
    painter.setPen(pen);
    ...
    for (Region::ConstIterator it(selection->constBegin()); it != end; ++it) {
        ...
        QColor selectionColor(QGuiApplication::palette().highlight().color());
