...
void CellView::paintCellBackground(QPainter& painter, const QRegion &clipRegion, const QPointF& coordinate) const
{
    ...
    if (d->style.backgroundColor().isValid() &&
            d->style.backgroundColor() != QGuiApplication::palette().base().color()) {
                         ...
                         const Cell& cell) const
{
    ...
    if (!textColorPrint.isValid()) {
        ...
        if (bgColor.isValid()) {
            qreal contrast = 5; //AFA KColorUtils::contrastRatio(bgColor, textColorPrint);
            if (contrast < 3)
    ...
    if (!cell.link().isEmpty()) {
        tmpPen.setColor(QGuiApplication::palette().link().color());
...
QString CellView::textDisplaying(const QFontMetricsF& fm, const Cell& cell)
{
    ...
    if (style().wrapText() || d->richText) {
        ...
        int idxOfDecimal = d->displayText.indexOf(locale->decimalPoint());  //AFA
        ...
        for (int i = start; i >= 0; i--) {
            ...
            if (isNumeric) {
                ...
                if (i < idxOfDecimal) {
                    ...
                    tmp2.remove(locale->groupSeparator());  //AFA
