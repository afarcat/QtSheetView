...
// Sheets
//AFA #include "ActionOptionWidget.h"
...
//AFA #include <KoColorPopupAction.h>
...
// KF5
//AFA #include <kfind.h>
//AFA #include <kfontaction.h>
//AFA #include <kfontsizeaction.h>
//AFA #include <klocale.h>
...
#ifndef NDEBUG
//AFA #include <QTableView>
//AFA #include "SheetModel.h"
#endif

#define koIconWanted(x, y) koIcon(y)
#define KSelectAction QAction
#define KToggleAction QAction
#define KFontAction QAction
#define KFontSizeAction QAction
#define KoColorPopupAction QAction
        ...
        , d(new Private(this))
{
    ...
    d->findTopRow = 0;
    d->findBottomRow = 0;
    //AFA d->typeValue = FindOption::Value;
    //AFA d->directionValue = FindOption::Row;
    d->find = 0;
    d->replace = 0;
    d->replaceCommand = 0;
    ...
    action = new QAction(koIcon("cell-style"), i18n("Cell Format..."), this);   //AFA
    ...
    //AFA connect(action, SIGNAL(triggered(QString)), this, SLOT(setStyle(QString)));
    ...
    //AFA connect(action, SIGNAL(triggered(QString)), this, SLOT(font(QString)));

    action = new KFontSizeAction(i18n("Select Font Size"), this);
    action->setIconText(i18n("Font Size"));
    addAction("fontSize", action);
    //AFA connect(action, SIGNAL(fontSizeChanged(int)), this, SLOT(fontSize(int)));
    ...
    //AFA connect(action, SIGNAL(colorChanged(KoColor)), this, SLOT(changeTextColor(KoColor)));
    ...
    //AFA static_cast<KoColorPopupAction*>(action)->setCurrentColor(Qt::black);
    addAction("borderColor", action);
    //AFA connect(action, SIGNAL(colorChanged(KoColor)), this, SLOT(borderColor(KoColor)));
    ...
    //AFA connect(action, SIGNAL(colorChanged(KoColor)), this, SLOT(changeBackgroundColor(KoColor)));
    ...
    //AFA static_cast<KSelectAction*>(action)->setItems(functionList);
    //AFA static_cast<KSelectAction*>(action)->setComboWidth(80);
    //AFA static_cast<KSelectAction*>(action)->setCurrentItem(0);
    //AFA connect(action, SIGNAL(triggered(QString)), this, SLOT(formulaSelection(QString)));
    ...
    //AFA action = KStandardAction::cut(this, SLOT(cut()), this);
    action = new QAction(koIcon("edit-cut"), i18n("Cut"), this);
    connect(action, SIGNAL(triggered(bool)), this, SLOT(cut()));
    action->setToolTip(i18n("Move the cell object to the clipboard"));
    addAction("cut", action);

    //AFA action = KStandardAction::copy(this, SLOT(copy()), this);
    action = new QAction(koIcon("edit-copy"), i18n("Copy"), this);
    ...
    //AFA action = KStandardAction::paste(this, SLOT(paste()), this);
    action = new QAction(koIcon("edit-paste"), i18n("Paste"), this);
    connect(action, SIGNAL(triggered(bool)), this, SLOT(paste()));
    ...
    //AFA action = KStandardAction::selectAll(this, SLOT(selectAll()), this);
    action = new QAction(i18n("Select All"), this);
    ...
    //AFA action = KStandardAction::find(this, SLOT(find()), this);
    action = new QAction(i18n("Find"), this);
    connect(action, SIGNAL(triggered(bool)), this, SLOT(find()));
    addAction("edit_find", action);

    //AFA action = KStandardAction::findNext(this, SLOT(findNext()), this);
    action = new QAction(i18n("Find Next"), this);
    connect(action, SIGNAL(triggered(bool)), this, SLOT(findNext()));
    addAction("edit_find_next", action);

    //AFA action = KStandardAction::findPrev(this, SLOT(findPrevious()), this);
    action = new QAction(i18n("Find Previous"), this);
    connect(action, SIGNAL(triggered(bool)), this, SLOT(findPrevious()));
    addAction("edit_find_last", action);

    //AFA action = KStandardAction::replace(this, SLOT(replace()), this);
    action = new QAction(i18n("Replace"), this);
    connect(action, SIGNAL(triggered(bool)), this, SLOT(replace()));
    ...
    //AFA action = KStandardAction::spelling(this, SLOT(spellCheck()), this);
    action = new QAction(i18n("Spelling"), this);
    connect(action, SIGNAL(triggered(bool)), this, SLOT(spellCheck()));
...
void CellToolBase::inputMethodEvent(QInputMethodEvent * event)
{
    ...
    if (editor()) {
        QGuiApplication::sendEvent(editor()->widget(), event);
    }
}
...
void CellToolBase::activate(ToolActivation toolActivation, const QSet<KoShape*> &shapes)
{
    ...
    //AFA static_cast<KSelectAction*>(this->action("setStyle"))->setItems(styleManager->styleNames());
...
QList<QPointer<QWidget> >  CellToolBase::createOptionWidgets()
{
    ...
    for (int i = 0; i < widgetNodes.size(); ++i) {
        ...
        //AFA widgets.append(new ActionOptionWidget(this, e));
    }
    ...
    if (col > maxCol() || row > maxRow()) {
        ...
        if (event->button() == Qt::RightButton) {
            ...
            if (sheetView->cellView(col, row).hitTestFilterButton(cell, cellViewRect, event->pos())) {
                ...
                //AFA FilterPopup::showPopup(canvas()->canvasWidget(), cell, cellViewRect, &database);
                return 0; // Act directly; no further strategy needed.
            }
...
void CellToolBase::createStyleFromCell()
{
    ...
    while (true) {
        ...
        if (styleName.length() < 1) {
            //AFA KMessageBox::sorry(canvas()->canvasWidget(), i18n("The style name cannot be empty."));
            continue;
        }

        if (selection()->activeSheet()->map()->styleManager()->style(styleName) != 0) {
            //AFA KMessageBox::sorry(canvas()->canvasWidget(), i18n("A style with this name already exists."));
            continue;
        }
        break;
    }
...
void CellToolBase::changeTextColor(const KoColor &color)
{
    ...
    command->setFontColor(color/*AFA .toQColor()*/);
    command->add(*selection());
    command->execute(canvas());
}
...
{
    QColor color; //AFA = static_cast<KoColorPopupAction*>(action("borderColor"))->currentColor();
    StyleCommand* command = new StyleCommand();
    command->setSheet(selection()->activeSheet());
...
void CellToolBase::borderColor(const KoColor &color)
{
    ...
    command->setSheet(selection()->activeSheet());
    command->setColor(color/*AFA .toQColor()*/);
    command->add(*selection());
    command->execute(canvas());
}
...
void CellToolBase::currency(bool enable)
{
    ...
    //AFA command->setPrecision(enable ?  selection()->activeSheet()->map()->calculationSettings()->locale()->monetaryDecimalPlaces() : 0);
...
void CellToolBase::changeBackgroundColor(const KoColor &color)
{
    ...
    command->setBackgroundColor(color/*AFA .toQColor()*/);
    command->add(*selection());
    command->execute(canvas());
}
...
{
    if (selection()->isRowSelected()) {
        //AFA KMessageBox::error(canvas()->canvasWidget(), i18n("Area is too large."));
        return;
    }
...
{
    if (selection()->isSingular()) {
        //AFA KMessageBox::error(canvas()->canvasWidget(), i18n("You must select multiple cells."));
        return;
    }
...
void CellToolBase::specialChar(QChar character, const QString& fontName)
{
    ...
    QGuiApplication::sendEvent(editor()->widget(), &keyEvent);
}
...
bool CellToolBase::paste()
{
    ...
    if (mimeData->hasFormat("application/vnd.oasis.opendocument.spreadsheet")) {
        ...
        //AFA if (!Odf::paste(buffer, map)) return false;
    }
...
{
    const QMimeData *const mimeData = QGuiApplication::clipboard()->mimeData();
...
Cell CellToolBase::nextFindValidCell(int col, int row)
{
    ...
    if (cell.isDefault() || cell.isPartOfMerged() || cell.isFormula())
        cell = Cell();
    //AFA if (d->typeValue == FindOption::Note && !cell.isNull() && cell.comment().isEmpty())
        cell = Cell();
    return cell;
}
