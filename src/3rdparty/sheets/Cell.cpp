...
//   debugSheetsFormula ;
    ...
    if (!formula().isValid()) {
        sheet()->showStatusMessage(i18n("Parsing of formula in cell %1 failed.").arg(fullName()));
        ...
        if (newStyleLoading) {
            ...
            // number ?
                ...
                */
                ...
                if (style().formatType() == Format::Percentage) {
                    if (value().isInteger())
                        t = locale->toString(value().asInteger() * 100);    //AFA
                    else
                        t = locale->toString((double)numToDouble(value().asFloat() * 100.0), 'g', precision); //AFA
                    setUserInput(pasteOperation(t, userInput(), op));
                    setUserInput(userInput() + '%');
                } else {
                    if (value().isInteger())
                        t = locale->toString(value().asInteger());
                    else
                        t = locale->toString((double)numToDouble(value().asFloat()), 'g', precision); //AFA
                    setUserInput(pasteOperation(t, userInput(), op));
                }
            }
            ...
            // date ?
                ...
                if (value().asDate(sheet()->map()->calculationSettings()).isValid())   // Should always be the case for new docs
                    setUserInput(locale()->toString(value().asDate(sheet()->map()->calculationSettings()), KLocale::ShortFormat)); //AFA
            ...
            // time ?
                ...
                if (value().asTime().isValid())    // Should always be the case for new docs
                    setUserInput(locale()->toString(value().asTime(), KLocale::ShortFormat)); //AFA
