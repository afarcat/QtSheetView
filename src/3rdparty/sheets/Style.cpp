...
QString Style::fontFamily() const
{
    if (!d->subStyles.contains(FontFamily))
        return qApp->font()/*AFA KoGlobal::defaultFont()*/.family(); // SubStyleOne<FontFamily, QString>().value1;
...
int Style::fontSize() const
{
    if (!d->subStyles.contains(FontSize))
        return qApp->font()/*AFA KoGlobal::defaultFont()*/.pointSize(); //SubStyleOne<FontSize, int>().value1;
