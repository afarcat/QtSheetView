...
bool AbstractRegionCommand::isApproved() const
{
    ...
    if (m_checkLock && m_sheet->cellStorage()->hasLockedCells(*this)) {
        //AFA KPassivePopup::message(i18n("Processing is not possible, because some "
        //AFA                             "cells are locked as elements of a matrix."),
        //AFA                        QGuiApplication::activeWindow());
        return false;
    }
            ...
            for (int col = range.left(); col <= range.right(); ++col) {
                for (int row = range.top(); row <= range.bottom(); ++row) {
                    ...
                    if (!cell.style().notProtected()) {
                        //AFA KPassivePopup::message(i18n("Processing is not possible, "
                        //AFA                             "because some cells are protected."),
                        //AFA                        QGuiApplication::activeWindow());
                        return false;
                    }
                }
