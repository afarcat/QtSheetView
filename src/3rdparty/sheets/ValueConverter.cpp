...
Value ValueConverter::asString(const Value &value) const
{
    ...
    case Value::Empty:
        ...
        break;
    case Value::Boolean: {
        //AFA const QStringList localeCodes(m_parser->settings()->locale()->country());
        val = Value(value.asBoolean() ? ki18n("True")/*AFA .toString(localeCodes)*/ :
                    ki18n("False")/*AFA .toString(localeCodes)*/);
        break;
    }
    case Value::Integer: {
        fmt = value.format();
        if (fmt == Value::fmt_Percent)
            ...
            val = Value(m_parser->settings()->locale()->toString(value.asDateTime(settings())));    //AFA
        else if (fmt == Value::fmt_Date)
            val = Value(m_parser->settings()->locale()->toString(value.asDate(settings())));  //AFA
        else if (fmt == Value::fmt_Time)
            val = Value(m_parser->settings()->locale()->toString(value.asTime()));    //AFA
        else
    ...
    case Value::Float:
        ...
        if (fmt == Value::fmt_DateTime)
            ...
            val = Value(m_parser->settings()->locale()->toString(value.asDate(settings()), KLocale::ShortFormat)); //AFA
        else if (fmt == Value::fmt_Time)
            val = Value(m_parser->settings()->locale()->toString(value.asTime())); //AFA
        else {
            //convert the number, change decimal point from English to local
            s = QString::number(numToDouble(value.asFloat()), 'g', 10);
            const QString decimalSymbol = m_parser->settings()->locale()->decimalPoint();   //AFA
