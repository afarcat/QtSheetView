        ...
        , d(new Private(this))
{
    d->workbook = map;
    if (map->doc()) {
        //AFA resourceManager()->setUndoStack(map->doc()->undoStack());
        //AFA QVariant variant;
        //AFA variant.setValue<void*>(map->doc()->sheetAccessModel());
        //AFA resourceManager()->setResource(75751149, variant); // duplicated in kochart.
    }
    d->model = new SheetModel(this);

    d->layoutDirection = QGuiApplication::layoutDirection();
...
void Sheet::addShape(KoShape* shape)
{
    ...
    //AFA shape->setApplicationData(new ShapeApplicationData());
...
QDomElement Sheet::saveXML(QDomDocument& dd)
{
    ...
    if (!pwd.isNull()) {
        if (pwd.size() > 0) {
            //AFA QByteArray str = KCodecs::base64Encode(pwd);
            QByteArray str = QByteArray::fromBase64(pwd);
