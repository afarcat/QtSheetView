...
{
    QGuiApplication::sendEvent(static_cast<Canvas*>(m_pCanvas), _ev);
}
...
void RowHeaderWidget::paintSizeIndicator(int mouseY)
{
    ...
    if (hu > 0.01)
        tmpSize = i18n("Height: %1 %2").arg(hu).arg(m_pCanvas->doc()->unit().symbol());
    else
        tmpSize = i18n("Hide Row");

    if (!m_lSize) {
        //AFA int screenNo = QGuiApplication::desktop()->screenNumber(this);
        m_lSize = new QLabel(nullptr/*QGuiApplication::desktop()->screen(screenNo)*/, Qt::ToolTip);
...
void ColumnHeaderWidget::paintSizeIndicator(int mouseX)
{
    ...
    if (wu > 0.01)
        tmpSize = i18n("Width: %1 %2").arg(wu).arg(m_pCanvas->doc()->unit().symbol());
    else
...
{
    QGuiApplication::sendEvent(static_cast<Canvas*>(m_canvasBase), _ev);
}
