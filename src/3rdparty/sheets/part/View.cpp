...
// Calligra includes
//AFA #include <KoComponentData.h>
//AFA #include <KoPluginLoader.h>
//AFA #include <KoGlobal.h>
//AFA #include <KoColor.h>
#include <KoCanvasWidget.h>
//AFA #include <KoMainWindow.h>
//AFA #include <KoShapeController.h>
//AFA #include <KoShapeManager.h>
//AFA #include <KoSelection.h>
//AFA #include <KoDockerManager.h>
#include <KoToolManager.h>
//AFA #include <KoTemplateCreateDia.h>
#include <KoXmlNS.h>
//AFA #include <KoZoomAction.h>
//AFA #include <KoZoomController.h>
#include <KoZoomHandler.h>
#include <KoToolProxy.h>
//AFA #include <KoModeBoxFactory.h>
...
//AFA #include "ShapeApplicationData.h"
#include "Sheet.h"
#include "Style.h"
...
// dialogs
//AFA #include "dialogs/PageLayoutDialog.h"
//AFA #include "dialogs/PreferenceDialog.h"
//AFA #include "dialogs/ShowDialog.h"
//AFA #include "dialogs/SheetPropertiesDialog.h"

// ui
#include "ui/CellView.h"
//AFA #include "ui/MapViewModel.h"
//AFA #include "ui/RightToLeftPaintingStrategy.h"
...
// D-Bus
#ifndef QT_NO_DBUS
//AFA #include "interfaces/ViewAdaptor.h"
//AFA #include <knotifyconfigwidget.h>
#endif
...
void View::Private::initActions()
{
    ...
    actions->open->setToolTip(i18n("Open document"));
    actionCollection["docOpen"] = actions->open;
    //connect(actions->open, SIGNAL(triggered(bool)), doc, SLOT(open()));

    actions->save = new QAction(koIcon("document-save"), i18n("Save document"), view);
    actions->save->setIconText(i18n("Save document"));
    actions->save->setToolTip(i18n("Save document"));
    actionCollection["docSave"] = actions->save;
    //connect(actions->save, SIGNAL(triggered(bool)), doc, SLOT(save()));

    actions->undo = new QAction(koIcon("edit-undo"), i18n("Undo"), view);
    actions->undo->setEnabled(false);
    actions->undo->setIconText(i18n("Edit undo"));
    actions->undo->setToolTip(i18n("Edit undo"));
    actionCollection["editUndo"] = actions->undo;
    connect(actions->undo, SIGNAL(triggered(bool)), doc->undoStack(), SLOT(undo()));
    connect(doc->undoStack(), SIGNAL(canUndoChanged(bool)), actions->undo, SLOT(setEnabled(bool)));

    actions->redo = new QAction(koIcon("edit-redo"), i18n("Redo"), view);
    actions->redo->setEnabled(false);
    actions->redo->setIconText(i18n("Edit redo"));
    actions->redo->setToolTip(i18n("Edit redo"));
...
View::View(/*AFA KoPart *part, */QWidget *_parent, Doc *_doc)
        : KoView(/*AFA part, _doc,*/ _parent)
        , d(new Private)
{
    ...
    //AFA setComponentName(Factory::global().componentName(), Factory::global().componentDisplayName());
    //AFA setXMLFile("calligrasheets.rc");
...
View::~View()
{
    ...
    KoToolManager::instance()->removeCanvasController(d->canvasWidget);
    ...
    //              The KoCanvasWidget takes ownership of the Canvas and does the deletion.
    delete d->canvasWidget;
    delete d;
}
...
void View::initView()
{
    ...
    KoCanvasWidget *canvasWidget = new KoCanvasWidget(this);
    d->canvasWidget = canvasWidget;
    d->canvasWidget->setCanvas(d->canvas);
    d->canvasWidget->setCanvasMode(KoCanvasWidget::Spreadsheet);
    canvasWidget->setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
    canvasWidget->setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
...
//            this, SLOT(setActiveSheet(Sheet*)));
    ...
    //AFA QVariant variant;
    //AFA variant.setValue<void*>(d->selection);
    //AFA d->canvas->resourceManager()->setResource(CanvasResource::Selection, variant);
    //AFA variant.setValue<QObject*>(doc()->map()->bindingManager());

    // Load the Calligra Sheets Tools
    ToolRegistry::instance()->loadTools();

    //AFA
    d->cellToolBar = new CellToolBar(this);
    d->cellToolBar->addAction(d->actions->undo);
    d->cellToolBar->addAction(d->actions->redo);

    d->cellToolBar->setCanvas(d->canvas);

    d->cellEditorWidget = new CellEditorWidget(this);
    d->cellEditorWidget->setCanvas(d->canvas);
    ...
    canvasWidget->setVerticalScrollBar(d->vertScrollBar);
    connect(d->vertScrollBar, SIGNAL(valueChanged(int)), canvasWidget, SLOT(updateCanvasOffsetY()));
    ...
    canvasWidget->setHorizontalScrollBar(d->horzScrollBar);
    connect(d->horzScrollBar, SIGNAL(valueChanged(int)), canvasWidget, SLOT(updateCanvasOffsetX()));
    ...
    d->viewLayout->setRowStretch(3, 10);
    d->viewLayout->addWidget(d->cellToolBar, 0, 0, 1, 3);       //AFA
    d->viewLayout->addWidget(d->cellEditorWidget, 1, 0, 1, 3);  //AFA
    d->viewLayout->addWidget(d->selectAllButton, 2, 0);
    d->viewLayout->addWidget(d->columnHeader, 2, 1, 1, 1);
    d->viewLayout->addWidget(d->rowHeader, 3, 0);
    d->viewLayout->addWidget(canvasWidget, 3, 1);
    d->viewLayout->addWidget(d->vertScrollBar, 2, 2, 2, 1, Qt::AlignHCenter);
    d->viewLayout->addWidget(bottomPart, 4, 0, 1, 2);
    d->viewLayout->setColumnMinimumWidth(2, extent);
    d->viewLayout->setRowMinimumHeight(4, extent);
...
KoCanvasWidget* View::canvasController() const
{
...
SheetView* View::sheetView(const Sheet* sheet) const
{
    ...
    if (!sheetView) {
        ...
        //AFA connect(sheetView, SIGNAL(visibleSizeChanged(QSizeF)),
        //AFA         d->zoomController, SLOT(setDocumentSize(QSizeF)));
...
void View::refreshSheetViews()
{
    ...
    foreach (SheetView *sheetView, sheetViews) {
        disconnect(sheetView, SIGNAL(visibleSizeChanged(QSizeF)),
                   d->canvas, SLOT(setDocumentSize(QSizeF)));
        //AFA disconnect(sheetView, SIGNAL(visibleSizeChanged(QSizeF)),
        //AFA            d->zoomController, SLOT(setDocumentSize(QSizeF)));
...
void View::initialPosition()
{
    ...
    //AFA d->mapViewModel->setActiveSheet(sheet);
...
{
    //AFA KoTemplateCreateDia::createTemplate(doc()->documentPart()->templatesResourcePath(), ".ots",
    //AFA                                     doc(), this);
}
...
void View::setActiveSheet(Sheet* sheet, bool updateSheet)
{
    ...
    if (!oldSheet || oldSheet->getShowFormula() != d->activeSheet->getShowFormula()) {
        ...
        //AFA stateChanged("show_formulas", showFormulas ? StateNoReverse : StateReverse);
    }
    ...
    //AFA d->canvas->resourceManager()->setResource(KoCanvasResourceManager::CurrentPage, QVariant(sheet->map()->indexOf(sheet) + 1));
    ...
    //AFA stateChanged("sheet_is_protected", protect ? StateNoReverse : StateReverse);
...
void View::changeSheet(const QString& _name)
{
    ...
    //AFA d->mapViewModel->setActiveSheet(t);
}
...
void View::insertSheet()
{
    if (doc()->map()->isProtected()) {
        //AFA KMessageBox::error(0, i18n("You cannot change a protected sheet."));
        return;
    }
...
{
    if (doc()->map()->isProtected()) {
        //AFA KMessageBox::error(this, i18n("You cannot change a protected sheet."));
        return;
    }
...
void View::hideSheet()
{
    ...
    if (doc()->map()->visibleSheets().count() ==  1) {
        //AFA KMessageBox::error(this, i18n("You cannot hide the last visible sheet."));
        return;
    }
...
void View::showSheet()
{
    ...
    //AFA ShowDialog dialog(this, d->selection);
    //AFA dialog.exec();
}
...
{  
#ifndef QT_NO_DBUS
    //AFA KNotifyConfigWidget::configure(this);
#endif
}

void View::preference()
{
    //AFA PreferenceDialog dialog(this);
    //AFA dialog.exec();
}
...
void View::setHeaderMinima()
{
    if (d->loading)   // "View Loading" not finished yet
        return;
    QFont font(qApp->font()/*AFA KoGlobal::defaultFont()*/);
...
void View::statusBarClicked(const QPoint&)
{
    ...
    //AFA if (factory())
    //AFA     if (QMenu* menu = dynamic_cast<QMenu*>(factory()->container("calc_popup" , this)))
    //AFA         menu->popup(mousepos);
}
...
void View::addSheet(Sheet *sheet)
{
    ...
    //AFA connect(sheet, SIGNAL(shapeAdded(Sheet*,KoShape*)),
    //AFA         d->mapViewModel, SLOT(addShape(Sheet*,KoShape*)));
    //AFA connect(sheet, SIGNAL(shapeRemoved(Sheet*,KoShape*)),
    //AFA         d->mapViewModel, SLOT(removeShape(Sheet*,KoShape*)));
}
...
#endif
    // Disconnect signals.
    //AFA disconnect(sheet, 0, d->mapViewModel, 0);
}

QColor View::borderColor() const
{
    return QColor(Qt::gray);//AFA d->canvas->resourceManager()->foregroundColor().toQColor();
}
...
void View::slotAutoScroll()
{
    ...
    if (actuallyDoScroll) {
        ...
        QGuiApplication::postEvent(canvas(), event);
