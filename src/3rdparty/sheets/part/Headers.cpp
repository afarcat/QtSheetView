...
RowHeader::RowHeader(CanvasBase *_canvas)
        ...
        m_cellToolIsActive(true), m_font(qApp->font()/*AFA KoGlobal::defaultFont()*/)
{
}
        ...
        if (m_frozen) {
            ...
            if (hiddenRows.count() > 0) {
                if (m_pCanvas->selection()->isColumnSelected()) {
                    //AFA KMessageBox::error(/* XXX TODO this*/0, i18n("Area is too large."));
                    return;
                }
...
//     ElapsedTime et( "Painting vertical header", ElapsedTime::PrintOnlyTime );
    ...
    while (yPos <= paintRect.bottom() && y <= KS_rowMax) {
        const bool rowsSelected = m_pCanvas->selection()->isRowSelected(y); //AFA-change
        const bool selected = (selectedRows.contains(y) || rowsSelected); //AFA-change
        const bool highlighted = (!selected && (affectedRows.contains(y) || rowsSelected)); //AFA-change
    ...
    if (m_bResize) {
        ...
        if (m_frozen) {
            ...
            if (hiddenCols.count() > 0) {
                if (m_pCanvas->selection()->isRowSelected()) {
                    //AFA KMessageBox::error(0 /* XXX TODO this */, i18n("Area is too large."));
                    return;
                }
...
void ColumnHeader::resize(const QSizeF& size, const QSizeF& oldSize)
{
    ...
    // LTR interface)
    if (sheet->layoutDirection() == Qt::RightToLeft && !QGuiApplication::isRightToLeft()) {
        int dx = size.width() - oldSize.width();
        scroll(dx, 0);
    } else if (sheet->layoutDirection() == Qt::LeftToRight && QGuiApplication::isRightToLeft()) {
        int dx = size.width() - oldSize.width();
        scroll(-dx, 0);
    }
}
...
//     ElapsedTime et( "Painting horizontal header", ElapsedTime::PrintOnlyTime );
    ...
    while (xPos <= paintRect.right() && x <= KS_colMax) {
        bool isColumnSelected = m_pCanvas->selection()->isColumnSelected(x); //AFA-change
        bool selected = (selectedColumns.contains(x) || isColumnSelected); //AFA-change
        bool highlighted = (!selected && (affectedColumns.contains(x) || isColumnSelected)); //AFA-change
