...
void DependencyManager::updateAllDependencies(const Map* map, KoUpdater *updater)
{
    ...
    foreach(const Sheet* sheet, map->sheetList()) {
        for (int c = 0; c < sheet->formulaStorage()->count(); ++c, ++cellCurrent) {
            ...
            if (updater)
                ;//AFA updater->setProgress(int(qreal(cellCurrent) / qreal(cellsCount) * 50.));
        }
    }
    cellCurrent = 0;
    foreach(const Sheet* sheet, map->sheetList()) {
        for (int c = 0; c < sheet->formulaStorage()->count(); ++c, ++cellCurrent) {
            ...
            if (updater)
                ;//AFA updater->setProgress(50 + int(qreal(cellCurrent) / qreal(cellsCount) * 50.));
        }
    }

    if (updater)
        ;//AFA updater->setProgress(100);
}
