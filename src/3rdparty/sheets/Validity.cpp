...
bool Validity::testValidity(const Cell* cell) const
{
    ...
    if (!valid) {
        if (d->displayMessage) {
            switch (d->action) {
            case Stop:
                //AFA KMessageBox::error((QWidget*)0, d->message, d->title);
                break;
            case Warning:
                //AFA if (KMessageBox::warningYesNo((QWidget*)0, d->message, d->title) == KMessageBox::Yes) {
                    valid = true;
                //AFA }
                break;
            case Information:
                //AFA KMessageBox::information((QWidget*)0, d->message, d->title);
                valid = true;
                break;
            }
        }

        cell->sheet()->showStatusMessage(i18n("Validation for cell %1 failed").arg(cell->fullName()));
    }
    return valid;
}
