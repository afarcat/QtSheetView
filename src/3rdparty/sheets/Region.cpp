...
QSet<int> Region::columnsSelected() const
{
    ...
    for (ConstIterator it = d->cells.constBegin(); it != endOfList; ++it) {
        if ((*it)->isColumn()) {
            ...
            //AFA-change: When select rows very slow
            if ((range.left() == 1 && range.right() == KS_colMax)) {
                continue;
            }

            //AFA-change: Optimizing large data insertion
            if (range.width() > result.capacity()) {
                result.reserve(result.size() + range.width());
            }
...
QSet<int> Region::rowsSelected() const
{
    ...
    for (ConstIterator it = d->cells.constBegin(); it != endOfList; ++it) {
        if ((*it)->isRow()) {
            ...
            //AFA-change: When select columns very slow
            if ((range.top() == 1 && range.bottom() == KS_rowMax)) {
                continue;
            }

            //AFA-change: Optimizing large data insertion
            if (range.height() > result.capacity()) {
                result.reserve(result.size() + range.height());
            }
