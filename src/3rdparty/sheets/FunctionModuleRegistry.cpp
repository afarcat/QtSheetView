...
//AFA #include <kglobal.h>
//AFA #include <KSharedConfig>
...
void FunctionModuleRegistry::Private::registerFunctionModule(FunctionModule* module)
{
    ...
    //AFA const QString fileName = QStandardPaths::locate(QStandardPaths::GenericDataLocation,
    //AFA                                                 QStringLiteral("calligrasheets/functions/")+module->descriptionFileName());
    QString fileName = QStringLiteral(SHEETS_FUNCTIONS)+module->descriptionFileName();
    if (fileName.isEmpty()) {
...
//AFA
Q_GLOBAL_STATIC(FunctionModuleRegistry, s_instance)

FunctionModuleRegistry* FunctionModuleRegistry::instance()
{
    //AFA K_GLOBAL_STATIC(FunctionModuleRegistry, s_instance)
    return s_instance;
}
