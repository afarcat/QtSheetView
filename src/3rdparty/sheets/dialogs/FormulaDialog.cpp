...
FormulaDialog::FormulaDialog(QWidget* parent, Selection* selection, CellEditorBase* editor, const QString& formulaName)
        : KoDialog(parent)
{
    setWindowTitle(i18n("Function"));
    //AFA setButtons(Ok | Cancel);
    ...
    connect(buttonBox, SIGNAL(rejected()), this, SLOT(slotClose())); //AFA
    connect(buttonBox, SIGNAL(accepted()), this, SLOT(slotOk()));    //AFA
    ...
    */
    ...
    // Add auto completion.
    //AFA searchFunct->setCompletionMode(KCompletion::CompletionAuto);
    searchFunct->setCompleter(&listFunct/*AFA, true*/);
...
QString FormulaDialog::createParameter(const QString& _text, int param)
{
    ...
    case KSpread_Any: {
        bool isNumber;
        double tmp = m_selection->activeSheet()->map()->calculationSettings()->locale()->toDouble(_text, &isNumber);
        Q_UNUSED(tmp);
...
void FormulaDialog::slotActivated(const QString& category)
{
    ...
    listFunct.setModel(functionsModel);  //AFA
