...
//AFA #include <kcolorbutton.h>
//AFA #include <kcombobox.h>
//AFA #include <klineedit.h>

//AFA #include <KoIcon.h>
#include <KoCanvasBase.h>
//AFA #include <KoUnitDoubleSpinBox.h>
#include <KoUnit.h>
        ...
        m_dlg(dlg)
{
    ...
    m_parentBox = new KComboBox(/*AFA false, */groupBox);
    m_parentBox->clear();
...
void CellFormatDialog::init()
{
    ...
    //AFA setFaceType(KPageDialog::Tabbed);
    setMinimumWidth(600);
    ...
    connect(buttonBox, SIGNAL(rejected()), this, SLOT(close())); //AFA
}
...
void CellFormatPageFloat::slotChangeState()
{
    ...
    if (generic->isChecked() || number->isChecked() || percent->isChecked() ||
        ...
        list += i18n("System: ") + dlg->locale()->toString(QTime::currentTime(), KLocale::ShortFormat/*AFA false*/);
        list += i18n("System: ") + dlg->locale()->toString(QTime::currentTime(), KLocale::LongFormat/*AFA true*/);
...
void CellFormatPageFloat::init()
{
    ...
    list += i18n("System: ") + dlg->locale()->toString(QDate::currentDate(), KLocale::ShortFormat);   //AFA
    list += i18n("System: ") + dlg->locale()->toString(QDate::currentDate(), KLocale::LongFormat);    //AFA
...
{
    QStringList list;
    list += i18n("System: ") + dlg->locale()->toString(QDateTime::currentDateTime(), KLocale::ShortFormat); //AFA
    list += i18n("System: ") + dlg->locale()->toString(QDateTime::currentDateTime(), KLocale::LongFormat);  //AFA
    listFormat->addItems(list);
}
        ...
        : QWidget(parent)
{
    ...
    //AFA connect(textColorButton, SIGNAL(changed(QColor)),
    //AFA         this, SLOT(slotSetTextColor(QColor)));
...
void CellFormatPageFont::setCombos()
{
    ...
    //AFA textColorButton->setColor(textColor);
        ...
        dlg(_dlg)
{
    ...
    //AFA m_indent->setLineStepPt(10.0);
    //AFA m_indent->setUnit(dlg->selection()->canvas()->unit());
    //AFA m_indent->changeValue(dlg->indent);
    ...
    //AFA width->setUnit(dlg->selection()->canvas()->unit());
    ...
    defaultWidth->setText(i18n("Default width (%1 %2)").arg(dw).arg(dlg->selection()->canvas()->unit().symbol()));
    if (dlg->isRowSelected)
    ...
    //AFA height->setUnit(dlg->selection()->canvas()->unit());
    //to ensure, that we don't get rounding problems, we store the displayed value (for later check for changes)
    ...
    defaultHeight->setText(i18n("Default height (%1 %2)").arg(dh).arg(dlg->selection()->canvas()->unit().symbol()));
    if (dlg->isColumnSelected)
...
{
    //AFA connect(color, SIGNAL(changed(QColor)),
    //AFA         this, SLOT(slotSetColorButton(QColor)));
...
CellFormatPagePattern::CellFormatPagePattern(QWidget* parent, CellFormatDialog *_dlg) : QWidget(parent)
{
    ...
    //AFA bgColorButton->setColor(bgColor);
    //AFA connect(bgColorButton, SIGNAL(changed(QColor)),
    //AFA         this, SLOT(slotSetBackgroundColor(QColor)));
    ...
    //AFA color->setColor(dlg->brushColor);
...
{
    b_notAnyColor = true;
    //AFA bgColorButton->setColor(palette().base().color());
    QPalette palette = current->palette();
