...
//AFA #include <CalligraVersionWrapper.h>
#ifdef Q_OS_WIN
#include <windows.h>
#else
...
Value func_info(valVector args, ValueCalc *calc, FuncExtra *)
{
    ...
    if (type == "release")
        return Value(0/*AFA CalligraVersionWrapper::versionString()*/);

    if (type == "numfile") {
        /*AFA KoApplication *app = qobject_cast<KoApplication*>(qApp);
        if(! app) {
           return Value(0);
        } else {
