...
Value func_dollar(valVector args, ValueCalc *calc, FuncExtra *)
{
    ...
    QString s; //AFA = locale->formatMoney(value, locale->currencySymbol(), precision);
...
Value func_fixed(valVector args, ValueCalc *calc, FuncExtra *)
{
    ...
    else result.replace(pos, 1, locale->decimalPoint());    //AFA
    if (!no_commas)
        while (0 < (pos -= 3))
            result.insert(pos, locale->groupSeparator());   //AFA
...
Value func_numbervalue(valVector args, ValueCalc *calc, FuncExtra *)
{
    ...
    bool ok;
    double v = calc->settings()->locale()->toDouble(text, &ok);//AFA l.readNumber(text, &ok);
