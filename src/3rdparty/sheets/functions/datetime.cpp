...
Value func_edate(valVector args, ValueCalc *calc, FuncExtra *)
{
    ...
    date.addMonths(months); //AFA = calc->settings()->locale()->addMonths(date, months);
...
Value func_eomonth(valVector args, ValueCalc *calc, FuncExtra *)
{
    ...
    date.setDate/*AFA setYMD*/(date.year(), date.month(), date.daysInMonth());
...
Value func_years(valVector args, ValueCalc *calc, FuncExtra *)
{
    ...
    if ((date1.month() != 1) || (date1.day() != 1))
        date1.setDate/*AFA setYMD*/(date1.year() + 1, 1, 1);
    date2.setDate/*AFA setYMD*/(date2.year(), 1, 1);
...
Value func_months(valVector args, ValueCalc *calc, FuncExtra *)
{
    ...
    if (date1.month() == 12)
        ...
        date1.setDate/*AFA setYMD*/(date1.year(), date1.month() + 1, 1);
    date2.setDate/*AFA setYMD*/(date2.year(), date2.month(), 1);
...
Value func_weeks(valVector args, ValueCalc *calc, FuncExtra *)
{
    ...
    int weekStartDay = calc->settings()->locale()->firstDayOfWeek();    //AFA
...
Value func_monthname(valVector args, ValueCalc *calc, FuncExtra *)
{
    ...
    QString monthName = calc->settings()->locale()->monthName(number/*AFA,
