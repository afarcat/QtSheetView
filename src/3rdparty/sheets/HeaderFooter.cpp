...
QString HeaderFooter::completeHeading(const QString &_data, int _page, const QString &_sheet) const
{
    ...
    //AFA KoDocumentInfo* info = m_pSheet->doc()->documentInfo();
    QString full_name;
    QString email_addr;
    ...
    //AFA p = getpwuid(getuid());
    //AFA gethostname(hostname, sizeof(hostname));

#ifndef Q_OS_ANDROID
    if (full_name.isEmpty())
        ;//AFA full_name = p->pw_gecos;
#endif

    if (email_addr.isEmpty())
        ;//AFA email_addr = QString("%1@%2").arg(p->pw_name).arg(hostname);
