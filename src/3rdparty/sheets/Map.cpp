...
//AFA #include <kcodecs.h>
//AFA #include <kcompletion.h>

//AFA #include <KoGlobal.h>
//AFA #include <KoEmbeddedDocumentSaver.h>
//AFA #include <KoStyleManager.h>
//AFA #include <KoParagraphStyle.h>
...
public:
    ...
    //AFA KCompletion listCompletion;
};
        ...
        d(new Private)
{
    ...
    d->textStyleManager = nullptr; //AFA new KoStyleManager(this);
    ...
    d->defaultRowFormat->setHeight(font.pointSizeF() * 1.5);  //AFA-change
...
{
    QString sheetName(i18n("Sheet%1").arg(d->tableId++));
    if ( !name.isEmpty() )
        sheetName = name;
...
QDomElement Map::save(QDomDocument& doc)
{
    ...
    if (!password.isNull()) {
        if (password.size() > 0) {
            QByteArray str = QByteArray::fromBase64(password); //AFA KCodecs::base64Encode(password);
