...
// first because that will probably be the most common case
    ...
    Number money; //AFA-FIXME = m_settings->locale()->readMoney(strStripped, &ok);
    if (ok) {
        val = Value(money);
...
Value ValueParser::tryParseBool(const QString& str, bool *ok) const
{
    ...
    //AFA const QStringList localeCodes(m_settings->locale()->country());

    if ((lowerStr == "true") ||
            (lowerStr == ki18n("true")/*AFA .toString(localeCodes)*/.toLower())) {
        val = Value(true);
        if (ok) *ok = true;
    } else if ((lowerStr == "false") ||
               (lowerStr == ki18n("false")/*AFA .toString(localeCodes)*/.toLower())) {
        val = Value(false);
        if (ok) *ok = true;
    }
    return val;
...
Value ValueParser::readNumber(const QString& _str, bool *ok) const
{
    ...
    if (neg)
        str.remove(0, 1/*AFA m_settings->locale()->negativeSign().length()*/);
    ...
    */
    ...
    QString major;
    QString minor;
    if ((pos = str.indexOf(m_settings->locale()->decimalPoint())) != -1) {  //AFA
        major = str.left(pos);
        minor = str.mid(pos + 1/*AFA m_settings->locale()->decimalPoint().length()*/);
        isInt = false;
    ...
    int thlen = 1; //AFA m_settings->locale()->groupSeparator().length();
    int lastpos = 0;
    while ((pos = major.indexOf(m_settings->locale()->groupSeparator())) > 0) { //AFA
...
{
    bool valid = false;
    QDate tmpDate = m_settings->locale()->toDate(str/*, &valid*/);  valid = tmpDate.isValid(); //AFA
    if (!valid) {
        ...
        // before it (%m/%d).
        QString fmt = ""; //AFA m_settings->locale()->dateFormatShort();
        ...
        if (yearPos > -1) {
            ...
            tmpDate = m_settings->locale()->toDate(str, fmt/*, &valid*/);   valid = tmpDate.isValid(); //AFA
        }
    }
    if (valid) {
...
Value ValueParser::tryParseTime(const QString& str, bool *ok) const
{
    ...
    if (!valid) {
        //AFA const QStringList localeCodes(m_settings->locale()->country());
        const QString stringPm = ki18n("pm")/*AFA .toString(localeCodes)*/;
        const QString stringAm = ki18n("am")/*AFA .toString(localeCodes)*/;
        int pos = 0;
...
QDateTime ValueParser::readTime(const QString& intstr, bool withSeconds, bool* ok) const
{
    ...
    while (l > formatpos || sl > strpos) {
        ...
        case 'p': {
            //AFA const QStringList localeCodes(m_settings->locale()->country());
            QString s(ki18n("pm")/*AFA .toString(localeCodes)*/.toLower());
            int len = s.length();
            if (str.mid(strpos, len) == s) {
                pm = true;
                strpos += len;
            } else {
                s = ki18n("am")/*AFA .toString(localeCodes)*/.toLower();
                len = s.length();
                if (str.mid(strpos, len) == s) {
                    pm = false;
                    strpos += len;
        ...
        case 'S':
            ...
            if (second < 0 || second > 59)
                goto error;
            if (strpos < sl && str.indexOf(m_settings->locale()->decimalPoint()) == (int)strpos) {  //AFA
                strpos += 1; //AFA m_settings->locale()->decimalSymbol().length();
