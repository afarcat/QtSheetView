        ...
        const QString& _formatString, bool thousandsSep)
{
    ...
    switch (fmt) {
    case Format::Number:
        localizedNumber = m_converter->settings()->locale()->toString(val, 'g', p); //AFA
        break;
    case Format::Percentage:
        ...
        break;
    case Format::Money:
        // There is no substitute for this in KLocale (toCurrencyString cannot set precision) :(
        localizedNumber;//AFA = m_converter->settings()->locale()->formatMoney(val, currencySymbol.isEmpty() ? m_converter->settings()->locale()->currencySymbol() : currencySymbol, p);
        break;
    case Format::Scientific: {
        const QString decimalSymbol = m_converter->settings()->locale()->decimalPoint(); //AFA
        localizedNumber = QString::number(val, 'E', p);
        if ((pos = localizedNumber.indexOf('.')) != -1) {
            localizedNumber.replace(pos, 1, decimalSymbol);
        }
        // TODO: port to KLocale or other (icu?) formatting
    ...
    if ((floatFormat == Style::AlwaysSigned) && value >= 0)
        if (m_converter->settings()->locale()->positiveSign().isNull()) //AFA
    ...
    if (fmt != Format::Scientific) {
        ...
        if (precision == -1) {
            QString decimalSymbol = m_converter->settings()->locale()->decimalPoint(); //AFA
        ...
        if (!thousandsSep) {
            const QString separator = m_converter->settings()->locale()->groupSeparator(); //AFA
            if (!separator.isNull()) {
...
QString ValueFormatter::timeFormat(const QDateTime &_dt, Format::Type fmtType, const QString& formatString)
{
    ...
    if (fmtType == Format::Time)
        result = m_converter->settings()->locale()->toString(dt.time(), KLocale::LongFormat); //AFA
...
QString ValueFormatter::dateFormat(const QDate &date, Format::Type fmtType, const QString& formatString )
{
    ...
    if (fmtType == Format::ShortDate) {
        tmp = m_converter->settings()->locale()->toString(date, KLocale::ShortFormat); //AFA
    } else if (fmtType == Format::TextDate) {
        tmp = m_converter->settings()->locale()->toString(date, KLocale::LongFormat); //AFA
    } else if (fmtType == Format::Date1) { /*18-Feb-99 */
        tmp = QString().sprintf("%02d", date.day()) +
              '-' + m_converter->settings()->locale()->toString(date, /*AFA KLocale::Month, */KLocale::ShortFormat) +
        ...
        tmp = m_converter->settings()->locale()->toString(date, /*AFA KLocale::Month, */KLocale::ShortFormat) +
              '-' + QString::number(date.year()).right(2);
    } else if (fmtType == Format::Date8) { /*February-99 */
        tmp = m_converter->settings()->locale()->toString(date, /*AFA KLocale::Month, */KLocale::LongFormat) +
              '-' + QString::number(date.year()).right(2);
    } else if (fmtType == Format::Date9) { /*February-1999 */
        tmp = m_converter->settings()->locale()->toString(date, /*AFA KLocale::Month, */KLocale::LongFormat) +
              '-' + QString::number(date.year());
    } else if (fmtType == Format::Date10) { /*F-99 */
        tmp = m_converter->settings()->locale()->toString(date, /*AFA KLocale::Month, */KLocale::LongFormat).at(0) +
              '-' + QString::number(date.year()).right(2);
        ...
        tmp = QString().sprintf("%d", date.day()) +
              ' ' + m_converter->settings()->locale()->toString(date, /*AFA KLocale::Month, */KLocale::LongFormat) +
              ' ' + QString::number(date.year());
