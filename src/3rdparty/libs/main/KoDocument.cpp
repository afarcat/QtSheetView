...
//AFA #include "KoMainWindow.h" // XXX: remove
//AFA #include <kmessagebox.h> // XXX: remove
//AFA #include <KNotification> // XXX: remove

//AFA #include "KoComponentData.h"
//AFA #include "KoPart.h"
//AFA #include "KoEmbeddedDocumentSaver.h"
//AFA #include "KoFilterManager.h"
//AFA #include "KoFileDialog.h"
//AFA #include "KoDocumentInfo.h"
//AFA #include "KoView.h"

//AFA #include "KoOdfReadStore.h"
//AFA #include "KoOdfWriteStore.h"
#include "KoXmlNS.h"

//AFA #include <KoProgressProxy.h>
//AFA #include <KoProgressUpdater.h>
//AFA #include <KoUpdater.h>
//AFA #include <KoDocumentRdfBase.h>
#include <KoDpi.h>
#include <KoUnit.h>
#include <KoXmlWriter.h>
//AFA #include <KoDocumentInfoDlg.h>
//AFA #include <KoPageLayout.h>
//AFA #include <KoGridData.h>
//AFA #include <KoGuidesData.h>

//AFA #include <kfileitem.h>
//AFA #include <KoNetAccess.h>
//AFA #include <klocalizedstring.h>
#include <MainDebug.h>
//AFA #include <kconfiggroup.h>
//AFA #include <kio/job.h>
//AFA #include <kdirnotify.h>
//AFA #include <KBackup>
...
//AFA #include "KoUndoStackAction.h"

//AFA #include <unistd.h>
...
public:
    ...
    //AFA KoFilterManager *filterManager; // The filter-manager to use when loading/saving [for the options]
    ...
    //AFA KoGridData gridData;
    //AFA KoGuidesData guidesData;

    bool isEmpty;

    //AFA KoPageLayout pageLayout;
    ...
    : d(new Private(this, parent))
{
    ...
    d->isEmpty = true;
    //AFA d->filterManager = new KoFilterManager(this, d->progressUpdater);

    connect(&d->autoSaveTimer, SIGNAL(timeout()), this, SLOT(slotAutoSave()));
    setAutoSave(defaultAutoSave());

    setObjectName(newObjectName());
    d->docInfo = nullptr; //AFA new KoDocumentInfo(this);

    //AFA d->pageLayout.width = 0;
    //AFA d->pageLayout.height = 0;
    //AFA d->pageLayout.topMargin = 0;
    //AFA d->pageLayout.bottomMargin = 0;
    //AFA d->pageLayout.leftMargin = 0;
    //AFA d->pageLayout.rightMargin = 0;

    d->undoStack = undoStack;
    d->undoStack->setParent(this);

    //AFA KConfigGroup cfgGrp(d->parentPart->componentData().config(), "Undo");
    //AFA d->undoStack->setUndoLimit(cfgGrp.readEntry("UndoLimit", 1000));
...
bool KoDocument::exportDocument(const QUrl &_url)
{
    ...
    //
    //AFA debugMain << "Restoring KoDocument state to before export";
...
bool KoDocument::saveFile()
{
    ...
    if (outputMimeType.isEmpty()) {
        ...
        //AFA debugMain << "Empty output mime type, saving to" << outputMimeType;
    }
    ...
    if (!isNativeFormat(outputMimeType)) {
        //AFA debugMain << "Saving to format" << outputMimeType << "in" << localFilePath();
        // Not native format : save using export filter
        //AFA KoFilter::ConversionStatus status = d->filterManager->exportDocument(localFilePath(), outputMimeType);
        //AFA ret = status == KoFilter::OK;
        //AFA suppressErrorDialog = (status == KoFilter::UserCancelled || status == KoFilter::BadConversionGraph);
    } else {
    ...
    if (!ret) {
        if (!suppressErrorDialog) {
            if (errorMessage().isEmpty()) {
                //AFA KMessageBox::error(0, i18n("Could not save\n%1", localFilePath()));
            } else if (errorMessage() != "USER_CANCELED") {
                //AFA KMessageBox::error(0, i18n("Could not save %1\nReason: %2", localFilePath(), errorMessage()));
            }
    ...
    if (ret) {
        //AFA KNotification *notify = new KNotification("DocumentSaved");
        //AFA notify->setText(i18n("Document <i>%1</i> saved", url().url()));
        //AFA notify->addContext("url", url().url());
        //AFA QTimer::singleShot(0, notify, SLOT(sendEvent()));
    }
...
{
    return false; //AFA d->filterManager->getBatchMode();
}

void KoDocument::setSaveInBatchMode(const bool batchMode)
{
    //AFA d->filterManager->setBatchMode(batchMode);
}
...
void KoDocument::slotAutoSave()
{
    if (d->modified && d->modifiedAfterAutosave && !d->isLoading) {
        ...
        if (d->specialOutputFlag == SaveEncrypted && d->password.isNull()) {
            ...
            //AFA connect(this, SIGNAL(sigProgress(int)), d->parentPart->currentMainwindow(), SLOT(slotProgress(int)));
            ...
            d->autosaving = false;
            emit clearStatusBarMessage();
            //AFA disconnect(this, SIGNAL(sigProgress(int)), d->parentPart->currentMainwindow(), SLOT(slotProgress(int)));
...
void KoDocument::setReadWrite(bool readwrite)
{
    ...
    //AFA foreach(KoView *view, d->parentPart->views()) {
    //AFA     view->updateReadWrite(readwrite);
    //AFA }

    //AFA foreach(KoMainWindow *mainWindow, d->parentPart->mainWindows()) {
    //AFA     mainWindow->setReadWrite(readwrite);
    //AFA }
...
{
    //AFA debugMain << "Saving to OASIS format";
...
QString KoDocument::autoSaveFile(const QString & path) const
{
    ...
    if (path.isEmpty()) {
        ...
        retval = QString("%1/.%2-%3-%4-autosave%5").arg(QDir::tempPath()).arg(d->parentPart->objectName()/*AFA componentData().componentName()*/).arg(QGuiApplication::applicationPid()).arg(objectName()).arg(extension);
#else
        // On Linux, use a temp file in $HOME then. Mark it with the pid so two instances don't overwrite each other's autosave file
        retval = QString("%1/.%2-%3-%4-autosave%5").arg(QDir::homePath()).arg(d->parentPart->componentData().componentName()).arg(QGuiApplication::applicationPid()).arg(objectName()).arg(extension);
#endif
    } else {
...
bool KoDocument::openFile()
{
    ...
    if (!QFile::exists(localFilePath())) {
        ...
        if (d->autoErrorHandlingEnabled)
            ...
            ;// AFA KMessageBox::error(0, i18n("The file %1 does not exist.", localFilePath()));
        d->isLoading = false;
        return false;
    }
...
bool KoDocument::loadNativeFormat(const QString & file_)
{
    ...
    if (!fileInfo.isFile()) {
        ...
        if (!fileInfo2.exists() || !fileInfo2.isFile()) {
            d->lastErrorMessage = i18n("%1 is not a file.").arg(file_);
            return false;
        }
    }
    ...
    if (isRawXML) {
        ...
        int errorLine;
        int errorColumn;
        KoXmlDocument doc; //AFA = KoXmlDocument(true);
        bool res;
...
void KoDocument::setModified(bool mod)
{
    ...
    if (mod) {
        d->isEmpty = false;
        //AFA documentInfo()->updateParameters();
    }
...
int KoDocument::queryCloseDia()
{
    ...
    QString name;
    if (documentInfo()) {
        //AFA name = documentInfo()->aboutInfo("title");
    }
    if (name.isEmpty())
        name = url().fileName();
...
QString KoDocument::caption() const
{
    QString c;
    if (documentInfo()) {
        //AFA c = documentInfo()->aboutInfo("title");
    }
...
{
    return createDomDocument(d->parentPart->objectName()/*componentData().componentName()*/, tagName, version);
}
...
{
    if (errorMessage().isEmpty()) {
        //AFA KMessageBox::error(0, i18n("Could not open\n%1", localFilePath()));
    }
    else if (errorMessage() != "USER_CANCELED") {
        //AFA KMessageBox::error(0, i18n("Could not open %1\nReason: %2", localFilePath(), errorMessage()));
    }
}
