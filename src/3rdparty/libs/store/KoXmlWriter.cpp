...
void KoXmlWriter::addCompleteElement(QIODevice* indev)
{
    ...
    Q_ASSERT(openOk);
    if (!openOk) {
        //AFA warnStore << "Failed to re-open the device! wasOpen=" << wasOpen;
        return;
    }
...
{
    if (d->tags.isEmpty())
        ;//AFA warnStore << "EndElement() was called more times than startElement(). "
        //AFA              "The generated XML will be invalid! "
        //AFA              "Please report this bug (by saving the document to another format...)" << endl;
