...
#ifdef KOXML_USE_QDOM
    ownerDoc.appendChild(ownerDoc.importNode(node, true));  //AFA
#else
...
bool KoXml::setDocument(KoXmlDocument& doc, QIODevice* device,
                        ...
                        int* errorColumn)
{
    //AFA QXmlStreamReader reader(device);
    //AFA reader.setNamespaceProcessing(namespaceProcessing);
    bool result = doc.setContent(device, namespaceProcessing, errorMsg, errorLine, errorColumn);  //AFA
    return result;
}
