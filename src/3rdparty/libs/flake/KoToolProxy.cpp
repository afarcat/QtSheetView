...
void KoToolProxy::mousePressEvent(KoPointerEvent *ev)
{
    ...
    if (d->multiClickCount && d->multiClickTimeStamp.elapsed() < QGuiApplication::styleHints()->mouseDoubleClickInterval() && d->multiClickButton == ev->button()) {
        // One more multiclick;
        d->multiClickCount++;
    } else {
...
void KoToolProxyPrivate::setCanvasController(KoCanvasWidget *c)
{
    controller = c;
}
...
bool KoToolProxy::paste()
{
    ...
    if (!success) {
        ...
        if (!image.isNull()) {
            ...
            QList<QUrl> urls = QGuiApplication::clipboard()->mimeData()->urls();
