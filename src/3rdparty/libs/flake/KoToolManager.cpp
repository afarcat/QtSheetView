...
//AFA #include "KoShapeLayer.h"
//AFA #include "KoShapeRegistry.h"
//AFA #include "KoShapeManager.h"
#include "KoCanvasBase.h"
//AFA #include "KoInputDeviceHandlerRegistry.h"
//AFA #include "KoInputDeviceHandlerEvent.h"
...
{
public:
    CanvasData(KoCanvasWidget *cc, const KoInputDevice &id)
            : activeTool(0),
            canvas(cc),
            inputDevice(id),
            dummyToolWidget(0),
    ...
    void activateToolActions()
    {
        ...
        KActionCollection *canvasActionCollection = nullptr; //AFA canvas->actionCollection();
        ...
        //AFA canvasActionCollection->readSettings(); // The shortcuts might have been configured in the meantime.
    }
    ...
    void deactivateToolActions()
    {
        ...
        KActionCollection *ac = nullptr; //AFA canvas->actionCollection();
...
// helper method.
CanvasData *KoToolManager::Private::createCanvasData(KoCanvasWidget *controller, const KoInputDevice &device)
{
...
void KoToolManager::Private::setup()
{
    ...
    //AFA KoInputDeviceHandlerRegistry::instance();
}
...
void KoToolManager::Private::postSwitchTool(bool temporary)
{
    ...
    foreach(QList<CanvasData*> list, canvasses) {
        ...
        foreach(CanvasData *data, list) {
            if (first) {
                ;//AFA debugFlake << "Canvas" << canvasCount++;
            }
            //AFA debugFlake << "  +- Tool:" << data->activeToolId  << (data == canvasData ? " *" : "");
            first = false;
        }
    }
#endif
    ...
    KoCanvasWidget *canvasControllerWidget = dynamic_cast<KoCanvasWidget*>(canvasData->canvas);
...
void KoToolManager::Private::attachCanvas(KoCanvasWidget *controller)
{
    Q_ASSERT(controller);
...
void KoToolManager::Private::movedFocus(QWidget *from, QWidget *to)
{
    ...
    // Check if this app is about QWidget-based KoCanvasWidget canvasses
    ...
    // canvasWidget is set as focusproxy for KoCanvasWidget,
...
void KoToolManager::Private::registerToolProxy(KoToolProxy *proxy, KoCanvasBase *canvas)
{
    ...
    foreach(KoCanvasWidget *controller, canvasses.keys()) {
...
void KoToolManager::requestToolActivation(KoCanvasWidget * controller)
{
...
void KoToolManager::registerTools(KActionCollection *ac, KoCanvasWidget *controller)
{
    Q_ASSERT(controller);
    Q_ASSERT(ac);
...
void KoToolManager::addController(KoCanvasWidget *controller)
{
    Q_ASSERT(controller);
    if (d->canvasses.contains(controller))
        return;
    ...
    connect(controller, SIGNAL(destroyed(QObject*)), this, SLOT(attemptCanvasControllerRemoval(QObject*)));
    connect(controller, SIGNAL(canvasRemoved(KoCanvasWidget*)), this, SLOT(detachCanvas(KoCanvasWidget*)));
    connect(controller, SIGNAL(canvasSet(KoCanvasWidget*)), this, SLOT(attachCanvas(KoCanvasWidget*)));
}

void KoToolManager::removeCanvasController(KoCanvasWidget *controller)
{
    Q_ASSERT(controller);
    disconnect(controller, SIGNAL(canvasRemoved(KoCanvasWidget*)), this, SLOT(detachCanvas(KoCanvasWidget*)));
    disconnect(controller, SIGNAL(canvasSet(KoCanvasWidget*)), this, SLOT(attachCanvas(KoCanvasWidget*)));
    d->detachCanvas(controller);
}

void KoToolManager::attemptCanvasControllerRemoval(QObject* controller)
{
    KoCanvasWidget* controllerActual = qobject_cast<KoCanvasWidget*>(controller);
    if (controllerActual) {
        removeCanvasController(controllerActual);
    }
}

void KoToolManager::updateShapeControllerBase(KoShapeBasedDocumentBase *shapeController, KoCanvasWidget *canvasController)
{
...
{
    Q_ASSERT(canvas);
    foreach(KoCanvasWidget *controller, d->canvasses.keys()) {
        if (controller->canvas() == canvas) {
#ifdef ENABLE_SHAPE
...
KoCanvasWidget *KoToolManager::activeCanvasController() const
{
...
QPair<QString, KoToolBase*> KoToolManager::createTools(KoCanvasWidget *controller, ToolHelper *tool)
{
    ...
    //AFA debugFlake << "Creating tool" << tool->id() << ". Activated on:" << tool->activationShapeId() << ", prio:" << tool->priority();
